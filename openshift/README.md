...
$ oc new-app kafka-builder -p GITHUB_REF="v2.13-2.6.0" -p IMAGE_STREAM_VERSION="2.13-2.6.0"
```
...
kafka   172.30.1.1:5000/test/kafka      2.13-2.6.0      1 days ago
```

**NOTE**: If you want to use this local/private image from containers on other projects then use the "\<project\>/NAME" value as `SOURCE_IMAGE` parameter value, and use one value of "TAGS" as `KAFKA_VERSION` parameter value (e.g: test/kafka:2.13-2.6.0).
